...
parser.add_argument('--source', help='Image source, can be image file ("test.jpg"), \
                    image folder ("test_dir"), video file ("testvid.mp4"), RTSP stream URL ("rtsp://..."), \
                    index of USB camera ("usb0"), or index of Picamera ("picamera0")', 
                    default=rtsp_url)
...
if record:
    if source_type not in ['video','usb','rtsp']:
        print('Recording only works for video, camera, and RTSP stream sources. Please try again.')
        sys.exit(0)
    if not user_res:
...
elif source_type == 'video' or source_type == 'usb' or source_type == 'rtsp':
...
while True:
    ...
    elif source_type == 'video' or source_type == 'rtsp': # If source is a video or RTSP stream, load next frame
        ret, frame = cap.read()
        if not ret:
    ...
    if source_type == 'video' or source_type == 'usb' or source_type == 'picamera' or source_type == 'rtsp':
    ...
    if source_type == 'image' or source_type == 'folder':
        key = cv2.waitKey()
    elif source_type == 'video' or source_type == 'usb' or source_type == 'picamera' or source_type == 'rtsp':
        key = cv2.waitKey(5)
...
if source_type == 'video' or source_type == 'usb' or source_type == 'rtsp':
    cap.release()
elif source_type == 'picamera':
    cap.stop()
